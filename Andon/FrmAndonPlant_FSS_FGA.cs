...
namespace Andon
{
    public partial class FrmAndonPlant_FSS_FGA : FrmBase
    {
        public int idx_form;
        private string formTitle = "STOCKFIT && ASSEMBLY";
        ...
        string[] plantA1 = new string[4] { "01", "02", "03", "04" };
        string[] plantA3 = new string[4] { "09", "10", "11", "12" };
        string[] plantB = new string[4] { "17", "18", "19", "20" };
        string[] plantC = new string[4] { "21", "22", "23", "24" };
        string[] plantE = new string[4] { "31", "32", "33", "34" };
        //string[] plantF = new string[4] { "35", "36", "37", "38" };
        string[] plantG = new string[4] { "44", "45", "46", "47" };
        string andonLine = "0";
        string andonZone = "0";
        string andonTitle = "";
        string andonOP_CD = "";
        ...
        public FrmAndonPlant_FSS_FGA(int int_idx = 0)
        {
            InitializeComponent();
        ...
        private void FrmAndonPlantC_Load(object sender, EventArgs e)
        {
            ...
            setContentLayout(this.tblLayoutPlantBC, null);
            setLayout();
            ...
            try
            {
                ...
                if (dtGet != null && dtGet.Rows.Count > 0)
                {
                    ...
                    if (dt.Any())
                    {
                        ...
                        {
                            setContentLayout(this.tblLayoutPlantBC, dtLayout);
                        }
        ...
        private void setContentLayout(Control ctrl, DataTable dtTbl)
        {
            ...
            {
                if (c.Name.ToUpper().Contains("FSS") || c.Name.ToUpper().Contains("FGA"))
                {
                    c.Text = "";
                    c.BackColor = Color.White;
                }

                if (c.Name.ToUpper().Contains("TITLE_FS") || c.Name.ToUpper().Contains("TITLE_FG"))
                {
                    string str = c.Name.Substring(c.Name.Length - 1, 1);
                ...
                if (dtTbl != null && dtTbl.Rows.Count > 0)
                {
                    ...
                    var dt = dtTbl.AsEnumerable().Where(row => row.Field<string>("OP_CD") == (c.Name.ToUpper().Contains("FSS") ? "FSS" : (c.Name.ToUpper().Contains("FGA") ? "FGA" : "-"))
                                    && row.Field<string>("MACHINE_LINE") == int.Parse(arrAssyLine[int.Parse(c.Name.Substring(chk, 1)) - 1]).ToString()
