...
namespace Andon
{
    public partial class FrmAndonPlantBC_UPC_UPN : FrmBase
    {
        public int idx_form;
        private string formTitle = "CUTTING && NO SEW";
        ...
        public FrmAndonPlantBC_UPC_UPN(int int_idx = 0)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                if (dtGet != null && dtGet.Rows.Count > 0)
                {
                    ...
                    if (dt.Any())
                    {
                        ...
                        {
                            setContentLayout(this.layoutPlantC_UPC_UPN, dtLayout);
                        }
        ...
        private void setContentLayout(Control ctrl, DataTable dtTbl)
        {
            ...
            string val = "";
            string str = "";
            string[] arrAssyLine = (Functions.location == "51B1" ? plantB : plantC);
            List<Control> allControls = Functions.GetAllControls(ctrl);
            foreach (Control c in allControls)
            {
                if (c.Name.ToUpper().Contains("UPC_") || c.Name.ToUpper().Contains("UPN_"))
                {
                    c.Text = "";
