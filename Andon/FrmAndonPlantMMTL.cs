...
namespace Andon
{
    public partial class FrmAndonPlantMMTL : FrmBase
    {
        public int idx_form;
        private string formTitle = "ECO LINE";
        ...
        public FrmAndonPlantMMTL(int int_idx = 0)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                if (dtGet != null && dtGet.Rows.Count > 0)
                {
                    ...
                    if (dt.Any())
                    {
                        ...
                        {
                            setContentLayout(this.layoutPlantMMTL, dtLayout);
                        }
        ...
        private void setContentLayout(Control ctrl, DataTable dtTbl)
        {
            ...
            {
                if (c.Name.ToUpper().Contains("FSS_") || c.Name.ToUpper().Contains("FGA_") || c.Name.ToUpper().Contains("UPS_") || c.Name.ToUpper().Contains("UPC_") || c.Name.ToUpper().Contains("UPN_"))
                {
                    c.Text = "";
                    c.BackColor = Color.White;
                }

                if (c.Name.ToUpper().Contains("TITLE_FS") || c.Name.ToUpper().Contains("TITLE_FG") || c.Name.ToUpper().Contains("TITLE_UPS")
                    || c.Name.ToUpper().Contains("TITLE_UPN") || c.Name.ToUpper().Contains("TITLE_UPC")
                   )
                {
                ...
                if (dtTbl != null && dtTbl.Rows.Count > 0)
                {
                    ...
                    if (chk > 1) str = c.Name.ToUpper().Substring(chk - 4, 3); else continue;
